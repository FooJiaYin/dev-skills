...
- **Manual flow without `wrap-up`:** `/verify` → `/update-docs` → `/code-review` → fix Critical/Warning findings (Suggestion/Nit are advisory) → `/report` → `/rename-session` → commit → deploy.
